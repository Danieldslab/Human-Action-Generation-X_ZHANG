    ...
    with torch.no_grad():
        for i, act in enumerate(dataset.act_name):
            ...
            for traj_np, label, fn_gt, fn_mask_gt in generator:
                ...
                Y_r = model.sample_prior(Y, label)
                ...
                for ii in range(args.bs):
                    ...
                    for jj in range(cfg.vae_specs['n_action']):
                        print(act,dataset.act_name[jj])
                        if(act!=dataset.act_name[jj]):
                            continue
                        for kk in range(2):
                            sequence = {'poses': y[:, ii,jj,kk], 'betas': betas}
