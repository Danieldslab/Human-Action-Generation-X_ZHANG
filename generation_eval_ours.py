...
def get_diversity_DTW(Y_r, fn, args, cfg,seq_l=None):
    ...
    yr_tmp[fn_mask_inv == 1] = 1e5
    yr = yr_tmp.reshape([-1, bs, cfg.vae_specs['n_action'], args.nk, traj_dim]). \
    ...
    if seq_l is None:
        seq_l = torch.where(fn.transpose(0, 1) == 1)[1].cpu().data.numpy().reshape([-1, args.nk]) + 1
    seq_l = seq_l
    seq1 = []
    seq2 = []
    seq_l_1 = np.array([])
    ...
    # print(seq1.shape,seq2.shape,Y_r.shape)
    cost, sl = batch_dtw_torch(seq1, seq2)
...
def val_wrong(epoch):
    ...
    accele_gt = 0
    diversity = 0
    diversity_perframe = np.zeros(dataset.max_len)
    diversity_perframe_rot = np.zeros(dataset.max_len)
    diversity_dtw = 0
    diversity_dtw_rot = 0
    ...
    with torch.no_grad():
        for act in dataset.act_name:
            ...
            for index,action_str,action_label,action_seq,action_len,fn_mask,fn_gt  in train_loader:
                ...
                Y = traj_tmp.clone().permute(1, 0, 2)#[t_his:]
                ...
                sample_action_str,sample_action_label = dataset._sample_label_(cfg.batch_size,act)
                ...
                label = sample_action_label#torch.eye(cfg.vae_specs['n_action'], device=device, dtype=dtype)
                # print(label.shape)
                # label = label[None, :, None, :].repeat([bs, 1, args.nk, 1]).reshape([-1, cfg.vae_specs['n_action']])
                # print(label.shape)
                Y_r = model.sample_prior(label)
                # Y_r = Y_r.permute(1, 0, 2)
                # Y_r = Y_r_sample.permute(1, 0, 2)
                                # print(dataset.traj_dim)
                ...
                #     traj_tmp = torch.clone(traj_tmp)
                #     index_used = list(range(30)) + list(range(36, 66))
                #     traj_tmp[:, :, index_used] = Y_r.permute(1, 0, 2)
                #     Y_r = traj_tmp.clone()                
                ...
                fn_gt = torch.zeros([1,dataset.max_len])#get_stop_sign(Y_r,args)
                # print(fn.shape,fn.device)
                fn_gt[:, dataset.max_len-1] = 1
                # print(fn_gt.shape,Y_r.shape)
                # exit(0)
                fn = fn_gt.repeat(256,1).transpose(0, 1)
                fn = fn.to(device) 
                # print(fn.shape,fn.device)
                # exit(0)
                seq_l = torch.where(fn.transpose(0, 1) == 1)[1].cpu().data.numpy()+1
                ...
                get diversity
                """
                yr = Y_r.reshape([-1, dataset.max_len, 60])
                mask = torch.tril(torch.ones([dataset.max_len, dataset.max_len], device=device)) == 0
                div_tmp = torch.cdist(yr, yr, p=2)[:, mask].mean(dim=-1)\
                    .reshape([-1, cfg.batch_size]).mean(dim=(0,1)).cpu().data.numpy()
                # print(yr.shape)
                # print(div_tmp)
                ...
                diversity_dtw += ((cost/sl).mean() * cost.shape[0])*label.shape[0]
...
def val(epoch):
    ...
    with torch.no_grad():
        ...
        for index,action_str,action_label,action_seq,action_len,fn_mask,fn_gt  in train_loader:
            ...
            traj_tmp = action_seq.type(dtype).squeeze().contiguous().permute(1, 0, 2) 
            seq_n, bs, dim = traj_tmp.shape
            # label = torch.eye(cfg.vae_specs['n_action'], device=device, dtype=dtype)[10,:].repeat(cfg.vae_specs['n_action'],1)
            # # print(label.shape)
            # # exit(0)
            # label = label[None, :, None, :].repeat([bs, 1, args.nk, 1]).reshape([-1, cfg.vae_specs['n_action']])
            label,label_class = dataset.get_render_label()
            sample_action_label = label[None, :, None, :].repeat([bs, 1, args.nk, 1]).reshape([-1, cfg.vae_specs['n_action']])
            sample_action_label_class = label_class[None, :, None, :].repeat([bs, 1, args.nk, 1]).reshape([-1, cfg.vae_specs['n_action_classes']])
            # sample_action_str = sample_action_str*(cfg.batch_size//cfg.vae_specs['n_action'])
            label = sample_action_label.clone()
            # print(sample_action_label.shape,sample_action_label_class.shape)

            sample_action_label = [sample_action_label,sample_action_label_class]
            Y_r = model.sample_prior(sample_action_label)
            # print(Y_r.shape)
            ...
            # # print(fn_gt.shape,Y_r.shape)
            # # exit(0)
            # fn = fn_gt.repeat(Y_r.shape[1],1).transpose(0, 1)
            # fn = fn.to(device) 
            fn = get_stop_sign(Y_r,args)
            # print(fn.shape)
            seq_l = torch.where(fn.transpose(0, 1) == 1)[1].cpu().data.numpy()+1
            ...
            get diversity
            """
            yr = Y_r.reshape([-1, bs, cfg.vae_specs['n_action'], args.nk, 60]). \
            ...
            if(False):
                ...
                print("cost min_value: ",cost.min()," ","cost max_value",cost.max(),"mean_value", (cost).mean())
                # print(sl,sl.shape)
                print("sl min_value: ",sl.min()," ","sl max_value",sl.max())
                # print(f"{time.time()-st1:.3f}")
                diversity_dtw += (cost).mean() * label.shape[0]
                print("diversity_dtw",diversity_dtw/total_num_sample,total_num_sample)

        print(f">>>> time used {time.time()-st:.3f}")
        logger.info(f">>>> action accuracy {acc_peract/total_peract:.3f}")
        print("lucky")
        # exit(0)
    ...
    mu2 = feat.mean(axis=0)
    cov2 = np.matmul(feat.transpose(1, 0), feat) / feat.shape[0]
    mu_cov_data = np.load(cfg_classifier.result_dir + f'/mu_cov.npz', allow_pickle=True)['data'].item()
    mu_test = mu_cov_data['mu_test']
    cov_test = mu_cov_data['cov_test']
    fid = calculate_frechet_distance(mu_test, cov_test, mu2, cov2)
    # logger.info(f' accuracy: {accuracy:.3f}, fid {fid:.3f}')
    mu_train = mu_cov_data['mu_train']
    cov_train = mu_cov_data['cov_train']
    fid2 = calculate_frechet_distance(mu_train, cov_train, mu2, cov2)
    logger.info(
        f'epo {args.iter} mode {args.mode} threshold {args.threshold} action classifier {args.cfg_classifier} accuracy: {accuracy:.3f}, test fid {fid:.3f}, train fid {fid2:.3f}, div_avg_per_frame {diversity_perframe.mean():.3f}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--class_iter', type=int, default=480)
    ...
    for seed in rand_seeds:
        ...
        """model"""
        model = get_action_vae_model(cfg, 60, max_len=dataset.max_len)
        ...
        cp_path = cfg_classifier.vae_model_path  % args.class_iter
        print(cp_path)
        print('loading model from checkpoint: %s' % cp_path)
        model_cp = pickle.load(open(cp_path, "rb"))
