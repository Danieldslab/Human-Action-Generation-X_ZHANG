...
class Config:
    ...
    def __init__(self, cfg_id, test=False):
        ...
        # create dirs
        self.base_dir = './results/tmp' if test else './results'
        self.exp_name = cfg.get("exp_name", cfg_id)
        self.cfg_dir = '%s/%s' % (self.base_dir, self.exp_name)
        self.model_dir = '%s/models' % self.cfg_dir
        self.result_dir = '%s/results' % self.cfg_dir
        self.data_dir = '%s/data' % self.cfg_dir
